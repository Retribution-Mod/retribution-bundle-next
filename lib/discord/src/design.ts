import { lookupModule, lookupModules } from '@retribution-mod/modules/finders'
import {
    withDependencies,
    withProps,
    ...
    () => {
        ...
        const [module] = lookupModule(
            withProps('TableRow', 'Button')
                ...
                .keyAs('Retribution.discord.design.Design'),
        )
...
export let FormSwitch: DiscordModules.Components.FormSwitch = proxify(() => {
    for (const [, id] of lookupModules(
        withDependencies(
            ...
            .keyAs('Retribution.discord.design.FormSwitch'),
        {
            initialize: false,
        },
    )) {
