...
import { exists, rm } from '@retribution-mod/modules/native/fs'
import { getErrorStack } from '@retribution-mod/utils/error'
import { sleepReject } from '@retribution-mod/utils/promise'
...
const { states: InitialPersistedStates, savedStates }: PersistedPluginStates =
    callNativeMethodSync('Retribution.plugins.states.read', [])
...
registerJSMethod('Retribution.plugins.states.update', (id, state) => {
    applyPluginFlags(
...
registerJSMethod(
    'Retribution.plugins.events.pluginErrored',
...
async function writePluginEnabledState(plugin: AnyPlugin, enabled: boolean) {
    const result = await callNativeMethod('Retribution.plugins.setEnabled', [
        plugin.manifest.id,
        enabled,
    ])
    ...
    await Promise.all(
        ...
        pListOrdered
            ...
            .map(async function runLate(plugin) {
                ...
                await callNativeMethod('Retribution.plugins.startNative', [
                    plugin.manifest.id,
                ])
...
export function requestNextBootDefaultsOnly() {
    callNativeMethodSync(
        'Retribution.plugins.states.requestNextBootDefaultsOnly',
        [],
    )
}
...
declare module '@retribution-mod/modules/native' {
    interface NativeMethods {
        'Retribution.plugins.startNative': [[id: PluginManifest['id']], null]
        'Retribution.plugins.states.read': [[], PersistedPluginStates]
        'Retribution.plugins.states.requestNextBootDefaultsOnly': [[], void]
        'Retribution.plugins.setEnabled': [
