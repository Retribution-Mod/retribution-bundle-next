...
import type { AnyFunction } from '@retribution-mod/utils/types'
import type {
    PluginLifecycles,
    PluginManifest,
    PluginOptions,
...
export function registerExternalPlugins() {
    registerJSMethod(
        'Retribution.plugins.events.pluginInstallResult',
    ...
    registerJSMethod(
        'Retribution.plugins.events.pluginUpdated',
    ...
    registerJSMethod(
        'Retribution.plugins.events.pluginInstallReady',
    ...
    const externals = callNativeMethodSync('Retribution.plugins.list', [])
    if (!externals) return
...
export async function uninstallExternalPlugin(plugin: AnyPlugin) {
    ...
    await callNativeMethod('Retribution.plugins.uninstall', [plugin.manifest.id])
...
export async function resyncPluginSources() {
    const externals = await callNativeMethod('Retribution.plugins.list', [])
    if (!externals) return
...
export function confirmInstall(
    ...
    return callNativeMethod('Retribution.plugins.confirmInstall', [token, accepted])
}
...
function createOptionsFactory(
    ...
    return () => {
        const opts = new Function(
            'Retribution',
            'plugin',
            `return ${script}\n//# sourceURL=Retribution:Plugin:${id}`,
        )(
            pUnscopedApi,
...
declare module '@retribution-mod/modules/native' {
    interface NativeMethods {
        'Retribution.plugins.list': [[], ExternalPlugin[] | null]
        'Retribution.plugins.installFile': [[], null]
        'Retribution.plugins.uninstall': [[string], null]
        'Retribution.plugins.confirmInstall': [
