import { TypedEventEmitter } from '@retribution-mod/discord/common/utils'
import { callNativeMethod, registerJSMethod } from '@retribution-mod/modules/native'
...
export function registerRepositoryEvents() {
    registerJSMethod(
        'Retribution.plugins.events.downloadProgress',
    ...
    registerJSMethod(
        'Retribution.plugins.events.repoStateUpdate',
...
export function listRepos(): Promise<Repo[]> {
    return callNativeMethod('Retribution.plugins.repos.list', [])
}

export function setRepos(config: RepoConfigEntry[]): Promise<null> {
    return callNativeMethod('Retribution.plugins.repos.set', [config])
}

export function refreshRepo(url: string): Promise<Repo> {
    return callNativeMethod('Retribution.plugins.repos.refresh', [url])
}
...
export function listRepoPlugins(url: string): Promise<RepoPluginListing[]> {
    return callNativeMethod('Retribution.plugins.repos.listPlugins', [url])
}
...
export function planInstall(
    ...
    return callNativeMethod('Retribution.plugins.planInstall', [
        id,
        version ?? null,
        channel ?? null,
        filteredRepos ?? null,
...
export function installFromRepo(plan: InstallPlan): Promise<{
    ...
    return callNativeMethod('Retribution.plugins.install', [plan])
}
...
export function listUpdates(url: string): Promise<RepoUpdate[]> {
    return callNativeMethod('Retribution.plugins.repos.listUpdates', [url])
}
...
declare module '@retribution-mod/modules/native' {
    interface NativeMethods {
        ...
        'Retribution.plugins.repos.set': [[config: RepoConfigEntry[]], null]
        'Retribution.plugins.repos.refresh': [[url: string], Repo]
        'Retribution.plugins.repos.listPlugins': [
            [url: string],
            RepoPluginListing[],
        ]
        'Retribution.plugins.repos.listUpdates': [[url: string], RepoUpdate[]]
        'Retribution.plugins.planInstall': [
            [
                id: string,
                version: string | null,
                channel: string | null,
