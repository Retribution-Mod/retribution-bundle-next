...
const TYPES_PACKAGE_NAME = '@retribution-mod/types'
 ...
 * so consumers only get them if they ask for them (`@retribution-mod/types/hidden`).
 *
...
/**
 * Rewrites `@retribution-mod/*` to its package-private `#lib/*`, **except** in module augmentation
 * headers (`declare module '@retribution-mod/*'`).
 *
...
async function internalizeSpecifiers(
    ...
    const pattern = new RegExp(
        `(declare\\s+module\\s+)?(['"])@retribution-mod/(${Array.from(entries)
    ...
    await Promise.all(
        files.map(async file => {
            ...
            for (const [, specifier] of code.matchAll(
                ...
                if (/^[.#]|^@retribution-mod\//.test(specifier!)) continue
...
async function generateIndex(entries: string[]): Promise<string> {
    ...
    for (const entry of entries) {
        ...
        const moduleName = entry.replace(/^lib\//, '@retribution-mod/')
