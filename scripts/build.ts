...
export default async function build(dev = Dev, log = true) {
        ...
        transform: {
            define: {
                __BUILD_DISCORD_SERVER_URL__: stringEnv(
                    'Retribution_DISCORD_SERVER_URL',
                ),
                __BUILD_SOURCE_REPOSITORY_URL__: stringEnv(
                    'Retribution_SOURCE_REPOSITORY_URL',
                ),
                __BUILD_LICENSE_URL__: stringEnv('Retribution_LICENSE_URL'),
                ...
                __BUILD_DEFAULT_PLUGIN_REPOSITORY_URL__: JSON.stringify(
                    stringEnv(
                        'Retribution_DEFAULT_PLUGIN_REPOSITORY_URL',
                        undefined,
                        false,
                    ),
                ),
                __BUILD_DONATE_URL__: JSON.stringify(
                    stringEnv('Retribution_DONATE_URL', undefined, false),
                ),

                // See types/build.d.ts for what these flags do
                __BUILD_FLAG_DEBUG_MODULE_LOOKUPS__: String(
                    boolEnv('Retribution_DEBUG_MODULE_LOOKUPS', dev),
                ),
                __BUILD_FLAG_DEBUG_MODULE_WAITS__: String(
                    boolEnv('Retribution_DEBUG_MODULE_WAITS', dev),
                ),
                __BUILD_FLAG_DEBUG_LAZY_VALUES__: String(
                    boolEnv('Retribution_DEBUG_LAZY_VALUES', false),
                ),
                __BUILD_FLAG_LOG_PROMISE_REJECTIONS__: String(
                    boolEnv('Retribution_LOG_PROMISE_REJECTIONS', dev),
                ),
            },
        },
    ...
    await bundle.write({
        ...
        file: 'dist/Retribution.js',
        format: 'iife',
        keepNames: true,
        postFooter: `//# sourceURL=Retribution`,
        topLevelVar: true,
    })
...
async function generateAssets() {
    ...
    for (const file of await readdir(AssetsDir)) {
        ...
        promises.push(
            writeFile(
                path2,
                `import{registerAsset}from'@retribution-mod/assets';import uri from${uriPath};const ${name}=registerAsset({name:'${name}',type:${type},uri});export { ${name} as default }`,
            ),
        )
    }
